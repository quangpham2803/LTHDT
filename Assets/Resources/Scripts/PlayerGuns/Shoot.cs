...
public class Shoot : MonoBehaviour {
    ...
    void Update() {
        ...
        if ((Input.GetButtonDown("Fire1") || Input.GetKeyDown(KeyCode.Space) || heldOn) && wait && Time.timeScale!=0 && remainingBulletsInCartridge>0 && !reloading) {
            ShootBullet();
            startTime = Time.time;
    ...
    // Instantiate bullet
    void ShootBullet() {
        GameObject bulletInstantiated = Instantiate(bullet, PlayerAim.instance.aimPoint.transform.position, PlayerAim.instance.aimPoint.transform.rotation);
        ...
        Destroy(bulletInstantiated, 0.1f);
